        ...
        if (!entities.empty())
        {
            ...
            for (auto &entity : entities)
            {
                ...
                if (i >= randomMax)
                {
                    ...
                    _character = _physics->AddCharacter(15, 16, 45, originComponent->Origin);
...
OriginComponent GenMapApp::BuildOriginComponent(
    ...
    int height)
{
    ...
    _projectionMatrix = glm::perspective(glm::radians(70.0f), float(width) / float(height), 0.1f, 4096.0f);
}
